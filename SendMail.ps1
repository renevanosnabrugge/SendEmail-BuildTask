...
[bool]$AttachToBodyBool = [System.Convert]::ToBoolean(($AddAttachmentToBody))
...
$AttachtoBody = ""
if ($AttachToBodyBool -eq $true) {
    $AttachToBody = (Get-Content $Attachment | Out-String)
    $BodyExpanded = $ExecutionContext.InvokeCommand.ExpandString($Body) + "$AttachToBody"
}

if ($AttachToBodyBool -eq $false) {
...
if($AttachToBodyBool)
{
    $MailParams.Add("AddAttachmentToBody",$true);
